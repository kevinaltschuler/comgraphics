 ...
 */
  ...
  public:
    ...
    if (animated) {
      ...
      modelView.top() = modelView.top() * animation
          * animation_transform
          * transform;
      if (child!=NULL)
     ...
     * \param graph a reference to the scenegraph object of which this tree is a pa//rt
     */
    ...
    std::vector<util::Light> getLights(stack<glm::mat4>& modelview) throw(runtime_error) {
        ...
        std::vector<util::Light> _lights = child->getLights(modelview);
