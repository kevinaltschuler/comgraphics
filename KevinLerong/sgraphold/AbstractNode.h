 ...
 */
  ...
  protected:
    ...
    std::vector<util::Light> nodeLights;
    ...
    void setAnimation(const glm::mat4& m) throw(runtime_error) {
        throw runtime_error(getName()+" is not a group node");
    }
    ...
    {
      nodeLights.push_back(l);
    }

    std::vector<util::Light> getLights(stack<glm::mat4>& modelview) throw(runtime_error) {
        throw runtime_error(getName()+"is not a node that can have lights");
    }
